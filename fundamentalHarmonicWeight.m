...
for file = files'
    ...
    [averageHarominc, averageSound] = getAverageHarmonic(fileName);
    
    factorValue = getFundamentalHarmonicWeight(averageHarominc);
...
save('getFundamentalHarmonicWeightFactorValues', 'woodwindFactorValues', 'brassFactorValues', 'stringFactorValues');

%% Use factor values
if ~exist('woodwindFactorValues', 'var') || ~exist('brassFactorValues', 'var') || ~exist('stringFactorValues', 'var')
    load('getFundamentalHarmonicWeightFactorValues');
end
...
hold on;
title('fundamentalHarmonicWeightFactorValues');
ylabel('Fundamental Harmonic Weight Ratio');

...
for file = files'
    ...
    try
        [averagePhase, averageSound] = getAveragePhase(fileName, note);
        factorValue = getPhaseImpact(averagePhase);
    catch
        warning(fileName);
        factorValue = NaN;
    end
...
save('phaseImpactFactorValues', 'woodwindFactorValues', 'brassFactorValues', 'stringFactorValues');

%% Use factor values
if ~exist('woodwindFactorValues', 'var') || ~exist('brassFactorValues', 'var') || ~exist('stringFactorValues', 'var')
    load('phaseImpactFactorValues');
end
...
hold on;
title('Phase Impact Factor');
ylabel('Phase Impact Factor');
xlabel('sample #');
legend('Woodwind', 'Brass', 'String', 'Woodwind Average', 'Brass Average', 'String Average');

saveas(image, 'phaseImpact.jpg');
